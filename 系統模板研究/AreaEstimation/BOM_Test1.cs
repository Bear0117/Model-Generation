...
public class BOM_Test1 : IExternalCommand
{
    ...
    public void Run(UIApplication uiapp)
    {
        ...
        if (selectedElement != null && selectedElement is Stairs)
        {
            ...
            //樓梯可以抓到的參數
            stairActualRiserHight = (double)UnitUtils.Convert(stair.LookupParameter("實際級高").AsDouble(), UnitTypeId.Feet, UnitTypeId.Meters);
            stairActualTreadDepth = (double)UnitUtils.Convert(stair.LookupParameter("實際級深").AsDouble(), UnitTypeId.Feet, UnitTypeId.Meters);
            ...
            foreach (StairsRun stairRun in stairRunsList)
            {
                ...
                {
                    stairPathLength += UnitsToMeters(stairPathCurve.Length);
                }
            }
                            ...
                            if (Math.Abs(dotProduct - 1.0) < 1e-9)
                            {
                                ...
                                foreach (EdgeArray edgeArray in riserTopFace.EdgeLoops)
                                {
                                    foreach (Edge edge in edgeArray)
                                    {
                                        ...
                                        {
                                            stairRunsWidth = UnitsToMeters(edgeLength);
                                            longestEdge = edge;
                                        }
                                    }
                                }
            ...
            foreach (StairsLanding stairLanding in stairLandingsList)
            {
                ...
                MessageBox.Show("實際級高:" + stairActualRiserHight.ToString()+"\n" + "平台高度:" + UnitsToMeters(stairLanding.BaseElevation).ToString());
                
                if (stairActualRiserHight == UnitsToMeters(stairLanding.BaseElevation))
                {
                    MessageBox.Show("in");
            ...
            double topFaceArea = 0;
            double bottomFaceArea = 0;
                        ...
                        if (geomObj is Solid solid)
                        {
                            ...
                            foreach (Face face in faces)
                            {
                                ...
                                if (Math.Abs(dotProduct - 1.0) < 1e-9)
                                {
                                    ...
                                    // 計算頂面的面積
                                    topFaceArea = SquareUnitToSquareMeter(topFace.Area);  
                                ...
                                else if (Math.Abs(dotProduct + 1.0) < 1e-9)
                                {
                                    ...
                                    // 計算底面的面積
                                    bottomFaceArea = SquareUnitToSquareMeter(bottomFace.Area);
        ...
        risersArea = (double)(stairRunsWidth * stairActualRiserHight * (stairActualNumRisers-2)) ;
    ...
    public static double UnitsToMeters(double value)
    {
